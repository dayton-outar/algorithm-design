# Data Structures and Algorithms in JavaScript

Lessons can be found in the following,

 - [Fundamentals of Data Structures and Algorithms](./structures-algorithms/)
 - [Discrete Mathematics](./discrete-mathematics/)
 - [Design Patterns & Philosophies](./patterns-philosophies/)
 - [Codility Lessons and Challenges](./lessons/codility/)
 - [LeetCode Problems and Solutions](./lessons/leetcode/)


## More Lessons

1. [Codility](https://app.codility.com/programmers/)
2. [Leetcode](https://leetcode.com/)

## Further Reading

1. Data Structures and Algorithms with JavaScript by Michael McMillan
2. JavaScript Data Structures and Algorithms by Sammie Bae
3. Introducing Regular Expressions by Michael Fitzgerald
4. [GeeksforGeeks: Data Structures & Algorithms Tutorials](https://www.geeksforgeeks.org/dsa/dsa-tutorial-learn-data-structures-and-algorithms/)
5. [DSA Visualizer - Interactive Data Structures & Algorithms](https://visualizedsa.com/)
6. [Performance of JavaScript .forEach, .map and .reduce vs for and for..of](https://leanylabs.com/blog/js-forEach-map-reduce-vs-for-for_of/)
7. [Difference between ?:, ?! and ?=](https://stackoverflow.com/questions/10804732/difference-between-and)